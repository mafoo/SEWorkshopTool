...
{
    public class SpaceGame : GameBase
    {
        ...
        protected override bool SetupBasicGameInfo()
        {
            ...
            MyVRageWindows.Init(MyPerGameSettings.BasicGameInfo.ApplicationName,
                MySandboxGame.Log, appDataPath, false);
            ...
            MyInitializer.InvokeBeforeRun(AppId, MyPerGameSettings.BasicGameInfo.ApplicationName + "ModTool",
                MyVRage.Platform.System.GetAppDataPath());
            ...
            m_steamService = MySteamGameService.Create(Game.IsDedicated, AppId);
            MyServiceManager.Instance.AddService(m_steamService);
            MyServiceManager.Instance.AddService(MySteamUgcService.Create(AppId, m_steamService));

            if (m_useModIO)
            {
                MySandboxGame.Log.WriteLineAndConsole("Using mod.io service, instead of Steam.");
                MyServiceManager.Instance.AddService(MyModIoService.Create(Game.IsDedicated,
                    MyServiceManager.Instance.GetService<IMyGameService>(), ModIO_GameName, ModIO_GameID, ModIO_Key,
                    ModIO_TestGameID, ModIO_TestKey, false));
            }
        ...
        {
            var assemblyname =
                Path.Combine(Path.GetDirectoryName(typeof(MySandboxGame).Assembly.Location),
                    "SpaceEngineers.exe");
            if (Assembly.ReflectionOnlyLoadFrom(assemblyname) is Assembly asm)
            {
                ...
                {
                    ModIO_GameName = program.GetField("MODIO_GAME_NAME", BindingFlags.NonPublic | BindingFlags.Static)
                        ?.GetRawConstantValue() as string;
                    ModIO_TestGameID =
                        program.GetField("MODIO_TEST_GAMEID", BindingFlags.NonPublic | BindingFlags.Static)
                            ?.GetRawConstantValue() as string;
                    ModIO_TestKey = program.GetField("MODIO_TEST_APIKEY", BindingFlags.NonPublic | BindingFlags.Static)
                        ?.GetRawConstantValue() as string;
                    ModIO_GameID = program.GetField("MODIO_LIVE_GAMEID", BindingFlags.NonPublic | BindingFlags.Static)
                        ?.GetRawConstantValue() as string;
                    ModIO_Key = program.GetField("MODIO_LIVE_APIKEY", BindingFlags.NonPublic | BindingFlags.Static)
                        ?.GetRawConstantValue() as string;
                }
                else
                {
                    MySandboxGame.Log.WriteLineAndConsole(string.Format(Constants.ERROR_Reflection,
                        "SpaceEngineers.MyProgram"));
                }
            }
            else
            {
        ...
        protected override void AuthenticateWorkshop()
        {
            if (m_useModIO)
            {
                ...
                if (string.IsNullOrEmpty(email))
                {
                    ...
                    Console.Write("Enter the email associated with your mod.io account: ");
                    email = Console.ReadLine();
                }

                if (string.IsNullOrEmpty(token))
                {
                    MyGameService.WorkshopService.RequestSecurityCode(email, (r) =>
                    {
                        if (r == MyGameServiceCallResult.OK)
                        {
                            Console.WriteLine(
                                "Mod.io has sent security code to your email. It expires after 15 minutes.");
                            Console.Write("Enter Security code: ");
                            var code = Console.ReadLine();

                            MyGameService.WorkshopService.AuthenticateWithSecurityCode(code,
                                (a) =>
                                {
                                    if (a == MyGameServiceCallResult.OK)
                                    {
                                        var clsMyModIo =
                                            typeof(MyModIoService).Assembly
                                                .GetType("VRage.Mod.Io.MyModIo");
                                        Console.WriteLine("Authentication successful!");

                                        var accessToken = clsMyModIo.GetField("m_authenticatedToken",
                                            BindingFlags.Static | BindingFlags.NonPublic)?.GetValue(null);
                                        token = (string) accessToken.GetType()
                                            .GetField("access_token", BindingFlags.Public | BindingFlags.Instance)
                                            ?.GetValue(accessToken);
                                        expires = (int) accessToken.GetType()
                                            .GetField("date_expires", BindingFlags.Public | BindingFlags.Instance)
                                            ?.GetValue(accessToken);
                                        ...
                                        MySandboxGame.Log.WriteLineAndConsole(
                                            $"Your authentication token has been saved in {config.FilePath}. Do not delete or replace this file, or you will need to authenticate again.");
        ...
        private void PostAuthentication(string token, int expires)
        {
            ...
            var access_token_t = typeof(MyModIoService).Assembly.GetType("VRage.Mod.Io.Data.AccessToken");
            var token_instance = Activator.CreateInstance(access_token_t);
            access_token_t.GetField("access_token", BindingFlags.Public | BindingFlags.Instance)
                ?.SetValue(token_instance, token);
            access_token_t.GetField("date_expires", BindingFlags.Public | BindingFlags.Instance)
                ?.SetValue(token_instance, expires);

            // Tell the game we're authenticated
            clsMyModIo.GetField("m_authenticatedToken", BindingFlags.Static | BindingFlags.NonPublic)
                ?.SetValue(null, token_instance);
            clsMyModIo.GetField("m_authenticated", BindingFlags.Static | BindingFlags.NonPublic)?.SetValue(null, 3);
            clsMyModIo.GetField("m_authenticatedUserId", BindingFlags.Static | BindingFlags.NonPublic)
                ?.SetValue(null, ulong.MaxValue);
        }
        ...
        private void ManuallyAddDLCs()
        {
            var obj = typeof(MyDLCs.MyDLC).GetConstructor(BindingFlags.NonPublic | BindingFlags.Instance, null,
                new Type[]
                {
                    typeof(uint), typeof(string), typeof(MyStringId), typeof(MyStringId), typeof(string),
            ...
            var dlcs = (Dictionary<uint, MyDLCs.MyDLC>) typeof(MyDLCs)
                .GetField("m_dlcs", BindingFlags.NonPublic | BindingFlags.Static).GetValue(null);
            var dlcsByName = (Dictionary<string, MyDLCs.MyDLC>) typeof(MyDLCs)
                .GetField("m_dlcsByName", BindingFlags.NonPublic | BindingFlags.Static).GetValue(null);
